...
const expectDateToBeEqual = (
  ...
  expect(parsedDate).not.toEqual(undefined);
  expect(parsedDate?.year).toEqual(expectedYear);
  expect(parsedDate?.month).toEqual(expectedMonth);
  expect(parsedDate?.day).toEqual(expectedDay);
};
...
describe("parseDateValue", () => {
  it("parse a 'YYYY-MM-DDThh:mm:ssZ' date", () => {
    ...
    expect(parsedDate?.hour).toEqual(0);
  });
  ...
  it("should parse time to the right timezone", async () => {
    ...
    expect(parsedDate?.hour).toEqual(21);
  });

  it.each([undefined, ""])("parse an empty or null date", (date) => {
    const parsedDate = parseDateValue(date);
    expect(parsedDate).toEqual(undefined);
  });
...
describe("parseRangeDateValue", () => {
  ...
  it.each([
    ...
    (start: string, end: string) => {
      expect(parseRangeDateValue([start, end])).toEqual(undefined);
    },
  );

  it("parse an undefined date range", () => {
    expect(parseRangeDateValue(undefined)).toEqual(undefined);
  });
...
describe("convertDateValueToString", () => {
  ...
  it("should return a UTC ISO 8601 string from a CalendarDate", async () => {
    ...
    const result = convertDateValueToString(date);
    expect(result).toEqual("2023-05-24T22:00:00.000Z");
  });
  ...
  it("should convert time to the right timezone", async () => {
    ...
    expect(result).toEqual("2023-05-25T03:00:00.000Z");
  });
