...
import { MenuItemBody } from "@gouvfr-lasuite/ui-components";
...
The styles are bundled in `@gouvfr-lasuite/ui-components/style`, so make sure it is imported once in your app.
...
```tsx
import { UserSearchFilter } from "@gouvfr-lasuite/ui-components";
