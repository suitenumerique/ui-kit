...
export const LaGaufreV2 = ({ apiUrl, data, ...props }: LaGaufreV2Props) => {
  ...
  const widgetPath =
    props.widgetPath || "https://static.suite.anct.gouv.fr/widgets/lagaufre.js";
  const defaultApiUrl = data?.services ? undefined : apiUrl;

  const label: string = props.label || t("components.laGaufre.label");
  const closeLabel: string =
    props.closeLabel || t("components.laGaufre.closeLabel");
  const loadingText: string =
    props.loadingText || t("components.laGaufre.loadingText");
  const newWindowLabelSuffix: string =
    props.newWindowLabelSuffix || t("components.laGaufre.newWindowLabelSuffix");
