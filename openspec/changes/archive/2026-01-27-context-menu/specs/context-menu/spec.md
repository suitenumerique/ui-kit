...
The `menu` prop SHALL accept either a static array of items or a function that receives context and returns items.

#### Scenario: Static menu items
- **WHEN** `menu` prop is an array of `ContextMenuItem`
- **THEN** those items SHALL be displayed in the menu

#### Scenario: Dynamic menu items with context
- **WHEN** `menu` prop is a function `(ctx) => ContextMenuItem[]`
...
- **WHEN** a menu item has `hidden: true`
- **THEN** that item SHALL NOT be rendered in the menu

#### Scenario: Disabled items
- **WHEN** a menu item has `disabled: true`
- **THEN** that item SHALL be visually indicated as disabled
- **AND** clicking it SHALL NOT trigger `onAction`

---

### Requirement: Menu item actions and variants

Each menu item SHALL support an `onAction` callback and optional `variant` for styling.

#### Scenario: Action triggered on click
- **WHEN** user clicks on a menu item
- **THEN** the item's `onAction` callback SHALL be invoked
...
- **THEN** the menu SHALL close after invoking `onAction`
...
- **THEN** the item's `onAction` SHALL be invoked
- **AND** the menu SHALL close
